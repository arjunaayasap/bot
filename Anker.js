...
const vcard = 'BEGIN:VCARD\n'
            + 'VERSION:3.0\n'
            + 'FN:ANKER\n'
            + 'ORG:Owner Mariposa BOT;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6281368646011:+62 813-6864-6011\n'
            + 'END:VCARD'

prefix = "!"
name = "~ M A R I P O S A B O T"
rdaftar = "TERIMA KASIH TELAH DAFTAR MENJADI TEMEN MARIPOSA😁"
rmenu = "HAI TEMEN MARIPOSA👋 SEMOGA SEHAT SELALU:)"
limitt = 10
ban = []
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["6285654444495@s.whatsapp.net"] // owner number ubah aja
		...
		case 'daftar':
					...
					if (isUser) return reply('kamu sudah Menjadi Temen MARIPOSA:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
		    ...
		    case 'map':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *MARIPOSA BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Tidak Terdaftar Di Dalam Database *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[MARIPOSA]','aqua'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
